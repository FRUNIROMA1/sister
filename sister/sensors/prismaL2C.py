...
def he5_to_envi(L2C_zip,out_dir,temp_dir,elev_dir,
    ...
    base_name = os.path.basename(L2C_zip)[12:-4]
    ...
    zip_base  =os.path.basename(L2C_zip)
    logging.info('Unzipping %s' % zip_base)
    with zipfile.ZipFile(L2C_zip,'r') as zipped:
        zipped.extractall(temp_dir)

    L2C_obj = h5py.File(f'{temp_dir}/PRS_L2C_STD_{base_name}.he5','r')
    version = L2C_obj.attrs['Processor_Version'].decode('UTF-8')
    ...
    vnir_data =  L2C_obj['HDFEOS']["SWATHS"]['PRS_L2C_HCO']['Data Fields']['VNIR_Cube']
    vnir_waves = L2C_obj.attrs.get('List_Cw_Vnir')
    vnir_fwhm = L2C_obj.attrs.get('List_Fwhm_Vnir')
    L2ScaleVnirMin  = L2C_obj.attrs.get('L2ScaleVnirMin')
    L2ScaleVnirMax = L2C_obj.attrs.get('L2ScaleVnirMax')

    rfl_dict = envi_header_dict ()
    rfl_dict['lines']= vnir_data.shape[0]
    rfl_dict['samples']= vnir_data.shape[2]
    rfl_dict['bands']=  vnir_data.shape[1]
    rfl_dict['wavelength']= vnir_waves
    rfl_dict['fwhm']= vnir_fwhm
    rfl_dict['interleave']= 'bsq'
    rfl_dict['data type'] = 12
    rfl_dict['wavelength units'] = "nanometers"
    rfl_dict['byte order'] = 0
    vnir_temp = f'{temp_dir}/PRS_{base_name}_{measurement}_vnir'

    writer = WriteENVI(vnir_temp,rfl_dict)
    writer.write_chunk(np.moveaxis(vnir_data[:,:,:],1,2), 0,0)

    # Export SWIR to temporary ENVI
    swir_data =  L2C_obj['HDFEOS']["SWATHS"]['PRS_L2C_HCO']['Data Fields']['SWIR_Cube']
    swir_waves = L2C_obj.attrs.get('List_Cw_Swir')
    swir_fwhm = L2C_obj.attrs.get('List_Fwhm_Swir')
    L2ScaleSwirMin  = L2C_obj.attrs.get('L2ScaleSwirMin')
    L2ScaleSwirMax = L2C_obj.attrs.get('L2ScaleSwirMax')

    rfl_dict['lines']= swir_data.shape[0]
    rfl_dict['samples']= swir_data.shape[2]
    rfl_dict['bands']=  swir_data.shape[1]
    rfl_dict['wavelength']= swir_waves
    rfl_dict['fwhm']= swir_fwhm
    swir_temp = f'{temp_dir}/PRS_{base_name}_{measurement}_swir'

    writer = WriteENVI(swir_temp,rfl_dict )
    ...
    rfl_dict  = envi_header_dict()
    rfl_dict ['description'] = f"Reflettance v{version} "
    rfl_dict ['lines']= vnir_obj.lines-4 #Clip edges of array
    rfl_dict ['samples']=vnir_obj.columns-4  #Clip edges of array
    rfl_dict ['bands']= len(vnir_waves.tolist() + swir_waves.tolist())
    rfl_dict ['wavelength']= vnir_waves.tolist() + swir_waves.tolist()
    rfl_dict ['fwhm']= vnir_fwhm.tolist() + swir_fwhm.tolist()
    rfl_dict ['interleave']= 'bil'
    rfl_dict ['data type'] = 4
    rfl_dict ['wavelength units'] = "nanometers"
    rfl_dict ['byte order'] = 0
    rfl_dict ['default bands'] = [int(vnir_obj.wave_to_band(850)),
                                  int(vnir_obj.wave_to_band(660)),
                                  int(vnir_obj.wave_to_band(560))]
   
    writer = WriteENVI(rfl_file,rfl_dict)
    iterator_v =vnir_obj.iterate(by = 'chunk',chunk_size=(1000,1000))
    iterator_s =swir_obj.iterate(by = 'chunk',chunk_size=(1000,1000))

    while not iterator_v.complete:
        chunk_v = iterator_v.read_next()[2:-2,2:-2,3:]
        chunk_v =L2ScaleVnirMin+np.flip(chunk_v,axis=2)*(L2ScaleVnirMax-L2ScaleVnirMin)/65535
        chunk_s = iterator_s.read_next()[2:-2,2:-2,:-6]
        chunk_s =L2ScaleSwirMin+np.flip(chunk_s,axis=2)*(L2ScaleSwirMax-L2ScaleSwirMin)/65535
        block = np.concatenate([chunk_v,chunk_s],axis=2)
        writer.write_chunk(block,0,0)
        

    #Load ancillary datasets
    geo =  L2C_obj['HDFEOS']["SWATHS"]['PRS_L2C_HCO']['Geolocation Fields']
    pvs =  L2C_obj['Info']["Ancillary"]['PVSdata']
    ...
    utc_time = np.ones(geo['Longitude'][:,:].shape[0]) *utc_time[:,0,np.newaxis]
    ...
    solar_az = solar.get_azimuth(geo['Latitude'][:,:],geo['Longitude'][:,:],mean_time)[2:-2,2:-2]
    solar_zn = 90-solar.get_altitude(geo['Latitude'][:,:],geo['Longitude'][:,:],mean_time)[2:-2,2:-2]
